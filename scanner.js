...
function getArtistId(artist) {
  ...
  return new Promise((resolve) => {
    db.get(
      "SELECT id FROM artist WHERE name = ? COLLATE NOCASE",
      [artist],
      (err, row) => {
        if (err) {
          console.error(err);
...
function getAlbumId(artist, album, year) {
  ...
  return new Promise((resolve) => {
    db.get(
      "SELECT id FROM album WHERE uniqname = ?",
      [uniqname],
      (err, row) => {
        if (err) {
          console.error(err);
          ...
          if (row) {
            ...
            db.run(
              "INSERT INTO album (name, uniqname) VALUES(?, ?)",
              [album, uniqname],
              (err) => {
...
function addFile(file, next) {
  (async () => {
    ...
    db.run(
      "INSERT INTO track (file, artist, album, year, title, track, disk, codec, bitrate, duration, lossless, genre) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)",
      [
        file,
        artistId,
        albumId,
...
function addFromArr(idx, done) {
  if (idx < filesToAdd.length) {
    ...
    if (!inDb) {
      newFiles++;
      console.log(
        `Adding file ${idx} of ${filesToAdd.length}: ${fileName} ...`
      );
      addFile(fileName, () => {
  ...
  () => {
    ...
    addFromArr(0, () => {
      if (filesToRemove.length) {
        ...
        db.run(
          "DELETE FROM track WHERE file IN (" +
            filesToRemove.map((e) => "?").join(",") +
            ")",
          filesToRemove,
          (err) => {
            if (err) {
...
function allDone() {
  console.log(
    `New files: ${newFiles} Existing files: ${filesInDb} Delted files: ${staleFiles}`
  );
  db.close();
